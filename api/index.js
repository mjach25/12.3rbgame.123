...
app.post('/api/track', async (req, res) => {
          try {
                        ...
                        await supabase.from('page_views').insert({ ip, user_agent: ua, page, visited_at: new Date().toISOString() });
                        res.json({ success: true });
...
app.get('/api/analytics', async (req, res) => {
          try {
              ...
              const [uTotal, uToday, uMonth, uYear] = await Promise.all([
                                supabase.from('page_views').select('ip'),
                                supabase.from('page_views').select('ip').gte('visited_at', todayStart),
                                supabase.from('page_views').select('ip').gte('visited_at', monthStart),
                                supabase.from('page_views').select('ip').gte('visited_at', yearStart),
                            ]);

              const unique = (arr) => arr.data ? new Set(arr.data.map(r => r.ip)).size : 0;
              ...
              const last7 = [];
                        for (let i = 6; i >= 0; i--) {
                                          ...
                                          const { count } = await supabase.from('page_views').select('*', { count: 'exact', head: true }).gte('visited_at', start).lt('visited_at', end);
                                          last7.push({ date: d.toLocaleDateString('en', { weekday: 'short', month: 'short', day: 'numeric' }), views: count || 0 });
                        }

              res.json({
                                views: { today: today.count || 0, month: month.count || 0, year: year.count || 0, total: total.count || 0 },
                                unique: { today: unique(uToday), month: unique(uMonth), year: unique(uYear), total: unique(uTotal) },
                                chart: last7
              });
          } catch(e) {
